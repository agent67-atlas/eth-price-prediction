...
"""
    ...
    for model, score in predictions['model_scores'].items():
        ...
        status = "✅ Excellent" if score > 0.90 else "✓ Good" if score > 0.70 else "⚠️ Fair"
...
def update_latest_readme(latest_dir, summary):
    ...
    summary_latest = summary.replace('_overview.png)', 'eth_prediction_overview.png)')
    summary_latest = summary_latest.replace('_1hour.png)', 'eth_1hour_prediction.png)')
    summary_latest = summary_latest.replace('_indicators.png)', 'eth_technical_indicators.png)')
